...
def load_data():  # 加载数据
    with open("cf/weight.txt", 'r') as f:  # 加载油价
        lines = f.readlines()
        # print(len(lines))
        for i in range(1, len(lines)):
            X[i-1][0] = float(lines[i].replace("\n", ""))

    with open("cf/kinds.txt", 'r') as f:   # 加载砂石运价
        lines = f.readlines()
        # print(len(lines))
        for i in range(1, len(lines)):
            X[i-1][1] = float(lines[i].replace("\n", ""))

    with open("cf/distance.txt", 'r') as f:   # 加载沿海运价综合指数
        lines = f.readlines()
        # print(len(lines))
        for i in range(1, len(lines)):
            X[i-1][2] = float(lines[i].replace("\n", ""))

    with open("cf/price.txt", 'r') as f:   # 加载沿海运价综合指数
        lines = f.readlines()
        # print(len(lines))
        for i in range(1, len(lines)):
            X[i-1][3] = float(lines[i].replace("\n", ""))

    with open("cf/cif.txt", 'r') as f:   # 加载长江综合航运运价指数
        lines = f.readlines()
        # print(len(lines))
        for i in range(1, len(lines)):
...
def divide_data(x_data, y_data, n):  # 把数据划分成训练集和验证集 使用留出法 分层抽样  季节分层
    ...
    for i, j in enumerate(x_data):
        if i<num_test:
            ...
            x_train[i-num_test] = j
    for i, j in enumerate(y_data):
        if i<num_test:
            y_test[i] = j
        else:
            y_train[i-num_test] = j
